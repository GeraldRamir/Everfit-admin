...
type RecipeItem = RecipeDetailItem;

const PAGE_SIZE = 8;

export default function RecetasAdminClient({
  items,
  summary,
}: {
  items: RecipeItem[];
  ...
  const [formInitial, setFormInitial] = useState<RecipeFormValues | null>(null);
  ...
  function openEdit(item: RecipeItem) {
    setFormInitial(recipeToFormValues(item));
    setFormOpen(true);
  }

  async function toggleFeatured(item: RecipeItem) {
    setBusyId(item.id);
    setToggling(true);
    await fetch(`/api/admin/recipes/${item.id}`, {
      method: "PATCH",
  ...
  async function togglePublished(item: RecipeItem) {
    setBusyId(item.id);
    await fetch(`/api/admin/recipes/${item.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  async function handleDelete(item: RecipeItem) {
    if (prefs.confirmDeletes && !confirm(`¿Eliminar la receta "${item.title}"?`)) return;
    setBusyId(item.id);
    setDeleting(true);
    await fetch(`/api/admin/recipes/${item.id}`, { method: "DELETE" });
    setDeleting(false);
    setBusyId(null);
    setSelectedId(null);
    router.refresh();
  ...
  return (
    <div>
      <AdminPageHeader
        title="Recetas"
        description="Crea y edita recetas fit con ingredientes, preparación y valores nutricionales."
        action={
      ...
      <div className="admin-table-wrap">
        ...
        {filtered.length === 0 ? (
          <div className="p-6">
            <EmptyState
              icon={Salad}
              title="No hay recetas publicadas"
              description="Usa «Nueva receta» para publicar la primera."
            />
          </div>
        ) : (
          <>
                    ...
                    >
                      ...
                      <td className="text-right">
                        <AdminRowActions
                          ...
                          commercialPath={`/recetas/${item.slug}`}
